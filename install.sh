...
# =====================================
# Installiert den MAS-Engineer aus der ZIP-Distribution
# in ~/.config/goose/, damit Goose ihn direkt nutzen kann.
#
# Nutzung:
...
# === QUELLEN (Distribution) — Struktur-Erkennung ===
if [ -f "$INSTALL_DIR/recipe/dev-mas-engineer.yaml" ]; then
    # Port-Modus: flache Struktur (MAS als Projekt initiiert)
    ...
    # Traditionell: MAS unter mas-engineer/
...
error()   { echo -e "${RED}${BOLD}FEHLER${NC} $1"; ERRORS=$((ERRORS + 1)); }
header()  { echo -e "\n${BLUE}${BOLD}━━━ $1 ━━━${NC}"; }
...
copy_file() {
    ...
    if ! [ -f "$src" ]; then error "Quelle nicht gefunden: $src"; return 1; fi
    ...
    if [ -f "$dst" ] && diff "$src" "$dst" > /dev/null 2>&1; then
        skip "${label}${name} (bereits aktuell)"; SKIPPED=$((SKIPPED + 1)); return
    fi
    cp "$src" "$dst"
...
validate_all() {
    ...
    if [ -f "$SRC_MAS_RECIPE" ]; then
        validate_yaml "$SRC_MAS_RECIPE" && ok "dev-mas-engineer.yaml ✅" || { error "dev-mas-engineer.yaml: YAML-Fehler"; errors=$((errors+1)); }
    else
        error "dev-mas-engineer.yaml fehlt! Kein MAS-Engineer zum Installieren."
        return 1
    fi
    ...
    ok "$sub_valid/$sub_count Sub-Agenten YAML-valide"
    ...
    if [ -f "$SRC_MAS_STATE/workflows.yaml" ]; then
        validate_yaml "$SRC_MAS_STATE/workflows.yaml" && ok "workflows.yaml ✅" || { error "workflows.yaml: YAML-Fehler"; errors=$((errors+1)); }
    fi
...
install_mas() {
    ...
    echo "  Quelle: $INSTALL_DIR/mas-engineer/"
    echo "  Ziel:   ~/.config/goose/"
    echo ""
    ...
    header "Backup (vorherige Installation)"
    ...
    if [ -d "$SRC_MAS_STATE/knowledge" ]; then
        if [ "$DRY_RUN" = true ]; then
            ...
            dry "📚 $k_count Wissensdateien → $DST_MAS_STATE/knowledge/"
            COPIED=$((COPIED + k_count))
        else
            mkdir -p "$DST_MAS_STATE/knowledge"
            for f in "$SRC_MAS_STATE/knowledge"/*.md; do
                [ -f "$f" ] && copy_file "$f" "$DST_MAS_STATE/knowledge/$(basename "$f")" "📚 Wissen: "
            done
        fi
    fi
...
show_status() {
    ...
    echo ""
    [ -f "$SRC_MAS_RECIPE" ] && ok "Distribution: dev-mas-engineer.yaml" || warn "Distribution: MAS fehlt!"
    [ -f "$DST_MAS_RECIPE" ] && ok "~/.config:   dev-mas-engineer.yaml" || warn "~/.config:   MAS nicht installiert"
    ...
    [ "$src_sub" -gt 0 ] && ok "Distribution: $src_sub Sub-Agenten" || warn "Distribution: Keine Sub-Agenten"
    [ "$dst_sub" -gt 0 ] && ok "~/.config:   $dst_sub Sub-Agenten" || warn "~/.config:   Keine Sub-Agenten installiert"
    ...
    [ "$dst_tools" -gt 0 ] && ok "~/.config:   $dst_tools Tools" || warn "~/.config:   Keine Tools installiert"
    
    [ -f "$SRC_MAS_STATE/workflows.yaml" ] && ok "Distribution: SOT (workflows.yaml)" || warn "Distribution: SOT fehlt"
    [ -f "$DST_MAS_STATE/workflows.yaml" ] && ok "~/.config:   SOT vorhanden" || warn "~/.config:   SOT nicht installiert"
}
...
show_help() {
    ...
    echo ""
    echo "Installiert den MAS-Engineer aus der ZIP-Distribution"
    echo "in ~/.config/goose/, damit Goose ihn direkt nutzen kann."
    echo ""
    echo -e "${BOLD}Schritte:${NC}"
    echo "  1. ZIP entpacken:  unzip mas-framework-*.zip -d ~/mas-install"
    echo "  2. Installieren:   cd ~/mas-install && ./install.sh"
    echo "  3. Goose starten:  goose run --recipe dev-mas-engineer"
    echo ""
...
main() {
    while [ $# -gt 0 ]; do
        case "$1" in
            --mas) shift ;;  # Default, wird ignoriert
            --dry-run|--test) DRY_RUN=true; shift ;;
            --status) show_status; exit 0 ;;
            --help|-h) show_help; exit 0 ;;
            *) echo -e "${RED}Unbekannt: $1${NC}"; show_help; exit 1 ;;
        esac
    done
    ...
    echo "╚════════════════════════════════════════════════════╝"
    echo -e "${NC}"
    echo "Richtung: Distribution  →  ~/.config/goose/"
    [ "$DRY_RUN" = true ] && echo -e "${YELLOW}Modus:  DRY-RUN${NC}"
    echo ""

    install_mas

    if [ "$DRY_RUN" = false ]; then
        [ $ERRORS -gt 0 ] && { error "$ERRORS Fehler"; exit 1; }
    fi
}
