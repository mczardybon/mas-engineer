...
├── sub_mas-demo-runner.yaml       # MAS-INTERNAL: runs research-team demo
├── sub_mas-analytics-reporter.yaml # MAS-INTERNAL
├── sub_mas-code-reviewer-{director,reporter,synthesizer,validator}.yaml  # PRE-EXISTING (pre-R32 multi-role)
└── sub_mas-cr-{*,*-orchestrator,*-coordinator,*-merger,*-prioritizer,*-crosschecker,*-scorer,*-formatter,*-recommender}.yaml  # R32 SPLIT (16 files)

Legacy archive: `recipe/sub/legacy/sub_mas-code-reviewer-*-ORIGINAL.yaml`
...
1. **Thin delegator** (original filename) — e.g. `sub_mas-cr-validator.yaml`
   - has same name as before, but stripped to 1 role + delegation
   - references orchestrator: "delegate to sub_mas-cr-validator-orchestrator"
2. **Orchestrator** (1 file) — `sub_mas-cr-validator-orchestrator.yaml`
   - coordinates the 2 sub-agents
3. **Sub-agents** (2 files) — `sub_mas-cr-validator-crosschecker.yaml`, `sub_mas-cr-validator-scorer.yaml`
...
9. Schedule updated: `grep "R32\|round: 32" .state/schedule.yaml`

## How to run demo-team e2e

`sub_mas-demo-runner.yaml` is **interactive** (asks R01 confirmation). Don't run it via `--no-session`. It will hang.
