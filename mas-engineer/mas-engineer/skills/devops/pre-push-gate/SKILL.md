...
- Writes `.state/pipeline/pre_push_validation.yaml`. If `status: blocked`, fix `blocked_reasons` first, then re-run.
...
`.state/pre-push-e2e-baseline.json` as `baseline_source`. This is
