...
2. Update `mas-engineer/.state/schedule.yaml` (round++, timestamp)
3. Update `mas-engineer/.state/changes.json` (add entry with run_id, patches, type, reason)
...
├── mas-engineer/.state/   # YAML configs, templates
│   ├── workflows.yaml
│   ├── best-practices.yaml
