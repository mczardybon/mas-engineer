...
2. Read .state/pipeline/self_audit.yaml
...
- **ONE LINE** added: `+ sub_mas-clone` (a dummy list entry)
...
`.state/health-report.json` and saw `{"checks":[],"score":0,
...
There are TWO writers of `.state/health-report.json`:
...
And `.mas/dashboards/data.json` (the "100/30") is written by
`tools/dev_dashboard_data.py`, which:
- Parses `sub/*.yaml` to count recipes (30)
- Reads `.state/agents.json` for LLM-self-reported `status: healthy`
...
4. **If you see "score:0" + "checks:[]" + "timestamp:null" + 53 bytes
   in a .state/*.json**: it's a stub. Don't include it in any
...
R110-34 (7c1ac57) was supposed to REMOVE `- sub_mas-clone` from
...
- `git show 7c1ac57 -- mas-engineer/recipe/dev-mas-engineer.yaml`
  showed `+ sub_mas-clone` (line ADDED, not removed)
