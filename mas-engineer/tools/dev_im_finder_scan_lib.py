#!/usr/bin/env python3
"""Library extracted from tools/dev_im_finder_scan.py — R110-411b.

This module contains ALL the import-safe helpers, constants, and the main
YAML scanner loop that the original dev_im_finder_scan.py defined at module
level. Importing this module does NOT trigger any file walk or side effect
(the YAML scan loop lives inside ``run_yaml_scan()`` which must be called
explicitly, and ``SCAN_DIRS`` / ``ALL_YAMLS`` constants are now derived
lazily via ``_collect_scope_dirs()``).

Public API (consumed by tools/dev_im_finder_scan.py CLI wrapper + tests):
  - SEVERITY_FILTER
  ...
  - _collect_scope_dirs
  - SCAN_DIRS, ALL_YAMLS        (lazy — call _collect_scope_dirs() first)
  - EXCLUDED_DIR_NAMES, EXCLUDED_PATH_PATTERNS
  - _INCLUDE_EXTERNAL, _USER_EXPLICIT_SCOPE
  - _is_path_excluded
  - findings, fid                (mutable scanner state — tests reset these)
  - add_finding
  - run_yaml_scan                (the original lines 254-739 main loop)

Everything that was ``module-level`` in the pre-R110-411b version is now
either:
  (a) a constant / function defined at module level here (side-effect-free),
  (b) inside ``run_yaml_scan()`` (the YAML walker — only runs when called).

# --- ORIGINAL CONTENT (lines 3-739 of pre-R110-411b dev_im_finder_scan.py) ---
# Lines 1-2 (shebang + first-line docstring) are skipped because the
# lib_docstring above already serves as the module docstring.
# These are the imports + constants + helper fns + main YAML scanner loop.
# The YAML scanner loop (lines 254-739) is now wrapped in run_yaml_scan()
# so that importing this module does NOT walk the filesystem.
...
# =====================================================================
# R110-411b: Module-level constants extracted from CLI body
# =====================================================================
# These regex / frozenset / dict constants were at module-level in the
# pre-R110-411b monolithic dev_im_finder_scan.py. They live at module
# level here so the check_* / _is_* functions (also extracted from CLI
# body, see below) can reference them without re-defining per call.
...
# NOTE: SD/RECIPE/COUNT constants are also defined below (lines ~85-180)
# because the original refactor9 extraction put them inline at the top
# of the file. We keep both copies for backwards-compat; Python uses
# the LAST binding on duplicate top-level assignments, which is
# intentional — both copies are identical, so the binding is stable.
...
def run_yaml_scan():
    """Run the YAML scanner loop (R110-411b wrapper).

    Pre-R110-411b, the YAML walker was module-level code that
    ran on `import dev_im_finder_scan`. R110-362 documented that
    this caused 30s+ pytest timeouts in test fixtures. R110-411b
    gates the walk behind this function + the `if __name__`
    guard at the bottom of the file.

    Tests that need the YAML walker should call this function
    explicitly (or invoke the CLI via subprocess).
    """
...
# --- R110-411b: script-mode guard ---
# Without this guard, `import dev_im_finder_scan_lib` would walk
# SCAN_DIRS at import-time (R110-362 lesson). Now the walk is
# gated behind `python3 tools/dev_im_finder_scan_lib.py`.
if __name__ == '__main__':
    run_yaml_scan()


# --- R110-411b: check_* + helper functions extracted from CLI body ---
# These were nested inside main() in the pre-R110-411b monolithic
# dev_im_finder_scan.py. R110-411b moves them to module-level in the
# lib so that tests can access them as ``mod.check_spec_drift`` etc.
