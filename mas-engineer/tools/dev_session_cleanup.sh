...
total = c.fetchone()[0]
c.execute(\"SELECT session_type, COUNT(*) FROM sessions GROUP BY session_type ORDER BY COUNT(*) DESC\")
print('  Typ-Verteilung:')
for t, n in c.fetchall():
    print(f'    {t:15s}: {n:3d} Sessions')
c.execute(\"SELECT session_type, MIN(created_at), MAX(created_at) FROM sessions GROUP BY session_type\")
print('  Datumsspanne:')
...
# ===== ANALYSE =====
echo -e "${YELLOW}🔍 Analysiere deletable Sessions...${NC}"
echo ""
...
c.execute("SELECT COUNT(*), SUM(COALESCE(total_tokens,0)) FROM sessions WHERE session_type='scheduled'")
...
c.execute("SELECT COUNT(*), SUM(COALESCE(total_tokens,0)) FROM sessions WHERE session_type='sub_agent' AND created_at < ?", (cutoff_str,))
...
c.execute("SELECT COUNT(*), SUM(COALESCE(total_tokens,0)) FROM sessions WHERE session_type='sub_agent' AND COALESCE(total_tokens,0) < 1000 AND created_at < ?", (cutoff_1h,))
...
c.execute("SELECT COUNT(*), SUM(COALESCE(total_tokens,0)) FROM sessions WHERE session_type='user' AND name LIKE 'Agent %'")
...
c.execute("SELECT COUNT(*), SUM(COALESCE(total_tokens,0)) FROM sessions WHERE session_type='user' AND (name = 'New Chat' OR name IS NULL)")
...
c.execute("SELECT COUNT(*), SUM(COALESCE(total_tokens,0)) FROM sessions WHERE session_type='user' AND name NOT LIKE 'Agent %' AND name != 'New Chat' AND name IS NOT NULL")
...
# 1. Scheduled
c.execute("DELETE FROM sessions WHERE session_type='scheduled'")
total += c.rowcount
print(f'  ✅ Deleted: scheduled = {c.rowcount} Sessions')

# 2. Health-Ping Sub-Agents (tokens < 1000, older als 1h)
c.execute("DELETE FROM sessions WHERE session_type='sub_agent' AND COALESCE(total_tokens,0) < 1000 AND created_at < ?", (cutoff_1h,))
total += c.rowcount
print(f'  ✅ Deleted: Health-Pings <1K Tokens = {c.rowcount} Sessions')

# 3. Sub-Agent older als KEEP_HOURS
c.execute("DELETE FROM sessions WHERE session_type='sub_agent' AND created_at < ?", (cutoff_str,))
total += c.rowcount
print(f'  ✅ Deleted: sub_agent >{keep_hours}h = {c.rowcount} Sessions')

# 4. Framework-User (Agent ...)
c.execute("DELETE FROM sessions WHERE session_type='user' AND name LIKE 'Agent %'")
total += c.rowcount
print(f'  ✅ Deleted: Agent-User = {c.rowcount} Sessions')

# 5. Empty New Chat
c.execute("DELETE FROM sessions WHERE session_type='user' AND (name = 'New Chat' OR name IS NULL)")
total += c.rowcount
