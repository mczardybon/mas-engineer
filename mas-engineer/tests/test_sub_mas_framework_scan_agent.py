...
def test_framework_scan_agent_4_sub_agents():
    """Spec: 4 sub-agents — scanner, auditor, finder, hardener."""
    content = RECIPE.read_text()
    for sub in ("sub_mas-framework-scanner", "sub_mas-framework-auditor",
                "sub_mas-framework-finder", "sub_mas-framework-hardener"):
        assert sub in content, \
            f"framework-scan-agent must delegate to: {sub}"


def test_framework_scan_agent_sub_recipes():
    """Orchestrator must have sub_recipes for all 4 framework agents."""
    with open(RECIPE) as f:
        data = yaml.safe_load(f)
