...
Specifically these lines were uncovered:
  ...
  - L188-191: relative-path indexing loop (R110-562: index every file's own
    relative path so instruction prose like 'recipe/sub/foo.yaml' resolves
    as "found" when the file actually exists, even when the file's content
    uses unquoted YAML `key: value` and never triggers the quoted/path/bare
    regexes).

R110-562 update: the old `_build_repo_literal_index(repo_root, exclude_path)`
two-arg signature was replaced by a one-arg `_build_repo_literal_index(repo_root)`.
The "exclude this file" semantic moved into _scan_pattern_b, which compares
the per-file literal count to the repo-wide count. Tests below reflect the
new contract: the index includes everything; staleness is decided downstream.
...
def _import_build_index():
    """Helper: load dev_self_audit._build_repo_literal_index with TOOLS on path."""
    sys.path.insert(0, str(TOOLS))
    try:
        from dev_self_audit import _build_repo_literal_index
    finally:
        sys.path.pop(0)
    return _build_repo_literal_index
...
def test_build_repo_literal_index_indexes_count_phrases(fixture_repo):
    ...
    _build_repo_literal_index = _import_build_index()
    # R110-562: no exclude_path kwarg — index is repo-wide.
    index = _build_repo_literal_index(fixture_repo)
...
def test_build_repo_literal_index_excludes_self_reference():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        _build_repo_literal_index = _import_build_index()
        index = _build_repo_literal_index(root)
...
def test_build_repo_literal_index_indexes_relative_paths(fixture_repo):
    """R110-562: every file's own relative path is indexed so that instruction
    prose mentioning a path like 'tools/fake1.py' resolves as 'found' even
    when the file's content never matches the quoted/path/bare regexes
    (e.g. unquoted YAML 'key: value' files)."""
    _build_repo_literal_index = _import_build_index()
    index = _build_repo_literal_index(fixture_repo)
    # Both files exist in the fixture_repo and their relative paths should
    # be indexed (count == 1, one entry per file).
    assert index.get("tools/fake1.py") == 1, \
        f"Expected 'tools/fake1.py' once, got {index.get('tools/fake1.py')}"
    assert index.get("recipe/sub_mas-demo.yaml") == 1, \
        f"Expected 'recipe/sub_mas-demo.yaml' once, got {index.get('recipe/sub_mas-demo.yaml')}"


def test_build_repo_literal_index_no_exclude_path_kwarg(fixture_repo):
    """R110-562 regression guard: the old 2-arg signature (repo_root,
    exclude_path) is gone — exclude_path was a per-file idempotent
    filter that caused 95x redundant glob walks. Calling with kwarg
    should now raise TypeError."""
    _build_repo_literal_index = _import_build_index()
    with pytest.raises(TypeError, match="exclude_path"):
        _build_repo_literal_index(fixture_repo, exclude_path=str(fixture_repo / "tools" / "fake1.py"))
