...
In `.mase/directives/STATUS.md` table, after line 48 (3c), add new
...
After DIREKTIVE 1+2+3:
  ...
  1. grep "3d" .mase/directives/STATUS.md → 1 match (the new row)
  2. grep "R110-124" .mase/directives/STATUS.md → ≥1 match in the
     3d row Effekt column
  3. wc -l .mase/directives/STATUS.md → original +3 lines (1 new
...
## SCOPE
  ...
  - mas-engineer/.mase/directives/STATUS.md (DIREKTIVE 1: +1 table
  ...
  - .mase/directives/STATUS.md (R110-125 entry)
  - .mase/directive_already_applied.json (marker)
...
python3 tools/dev_directive_applier.py --hook pre-apply \
  .mase/directives/R110-125-status-3d-changelog.md
...
# 3. verify
grep -c "| 3d " .mase/directives/STATUS.md
# Expected: 1
grep -c "R110-124" .mase/directives/STATUS.md
# Expected: >=1
