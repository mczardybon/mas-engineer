...
Erstelle 3 files:
  ...
  2. `recipe/instructions/sub_mas-self-audit.md`
     ...
     3 hook points: PRE-AUDIT (skip if .mase/pipeline/self_audit.
     yaml exists + fresh < 1h), POST-AUDIT (return count), ERROR
     (log to .mase/self_audit_failures.json).
...
.mase/workflows.yaml (MODIFIED, +sub_mas-self-audit in registry)
.mase/directives/STATUS.md (MODIFIED, PHASE 3 → DONE)
...
[archive today's entries to .mase/changes.archive-DATE.json]
...
echo "per directive .mase/directives/R110-118-self-audit-implementation.md apply DIREKTIVE 1+2+3: create sub_mas-self-audit agent + dev_spec_invariant.py + dev_self_audit.py + Check 18 + status update. ack" | \
...
test -f .mase/test_apply_directive_dispatch.log && echo "DISPATCH OK"
