...
sub_mas-apply-directive der:
  1. liest .mase/directives/R<NR>-*.md files
...
KONKRETE SPEZIFIKATION:
  ...
  3. STEPS (analog im-designer STEP 1-3):
     ...
     STEP 1: read .mase/directives/R<NR>-*.md (newest first per
     ...
     STEP 4: write .mase/pipeline/directive_patches.yaml
  ...
  4. NEUE DATEI: mas-engineer/tools/dev_directive_parser.py
     ...
     CLI: `python3 -m dev_directive_parser --directive <path>
           --output .mase/pipeline/directive_patches.yaml`
          exit 0 wenn parse OK, 1 sonst.

  5. NEUE DATEI: mas-engineer/tools/dev_directive_applier.py
     Standalone-script, nimmt .mase/pipeline/directive_patches.yaml
...
directive-file-path enthaelt (z.B. ".mase/directives/R110-109-*.md").
Beispiel: "FULL_IMPROVEMENT per directive .mase/directives/R110-109-
...
per directive .mase/directives/R110-109-*.md'" tatsaechlich
sub_mas-apply-directive summont (statt R04-block).
...
1+2 implementiert sind; DIREKTIVE 3 spec'd nur den test):
  1. NEUE TEST: mas-engineer/tests/test_sub_mas_apply_directive.py
     ...
     - test_dispatch_via_message: parse ".mase/directives/R110-109-*" aus
...
================================================================
  ...
  echo "FULL_IMPROVEMENT per directive .mase/directives/R110-113-apply-directive.md -
...
VERIFIKATION:
  ...
  - python3 -c "from dev_directive_parser import parse_directive;
                parse_directive('.mase/directives/R110-109-self-audit-spec-invariant.md');
...
ROLLBACK-STRATEGY:
  - Bei false-positive dispatch (v3 summont sub_mas-apply-directive
    ...
    enger fassen (z.B. nur wenn "per directive" + ".mase/directives/"
    beides im message)
  ...
  - Bei applier-fail: dev_directive_applier.py hat
    ...
    .mase/pipeline/directive_patches_failed.yaml
