...
`recipe/sub/sub_mas-general-improver.yaml`:
    ...
    +   description: "R110-113: applies operator-written .directives/ specs"
...
**Action:** Erstelle `.state/test_apply_directive_dispatch.log` mit
inhalt:
...
`.state/test_apply_directive_dispatch.log` (created by apply)
...
- File `.state/test_apply_directive_dispatch.log` existiert
...
python3 -c "import json; from datetime import datetime, timezone; \
  p='.state/changes.json'; d=json.load(open(p)) if __import__('os').path.exists(p) else []; \
  ...
  ar={'metadata':{'archived_at':datetime.now(timezone.utc).isoformat(),\
    ...
    'changes':td}; \
  open(f'.state/changes.archive-{today}.json','w').write(json.dumps(ar,indent=2)+chr(10)); \
...
echo "per directive .directives/R110-117-apply-directive-e2e-test.md apply" | \
...
test -f .state/test_apply_directive_dispatch.log && \
  echo "DISPATCH SUCCESS" || echo "DISPATCH FAILED"
cat .state/test_apply_directive_dispatch.log
