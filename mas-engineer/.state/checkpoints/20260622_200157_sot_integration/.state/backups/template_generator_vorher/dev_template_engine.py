#!/usr/bin/env python3
"""
dev_template_engine.py — Erstellt BP-konforme Sub-Agent YAMLs
"""
...
def extract_rules(bp, kategorie, max_rules=3):
    regeln = bp.get(kategorie, [])
    return [r.get('rule', '?') for r in regeln[:max_rules]]
...
def generate_yaml(name, emoji, task, output, workspace, mode='mas', auto_commit=False):
    ...
    prompt += "\n→ Gib Ergebnis als strukturierten Report zurueck"
    ...
    lines.append("Der MAS-Engineer uebergibt diese Parameter via delegate():")
    ...
    if auto_commit:
        prompt += '\n\n# AUTO-COMMIT AKTIV:\n'
        prompt += '# NACH Änderung: git add -A && git commit -m "[MAS] {action}"\n'
        prompt += '# checkpoint .state/checkpoints/{ts}/ + changes.json + todo\n'

    lines.append("")
    lines.append("## SCHRITTE:")
    lines.append("1. Lese Input")
    lines.append("2. Fuehre " + task + " durch")
    lines.append("3. Gib strukturierten Report zurueck")
    lines.append("")
    lines.append("## Output")
    ...
    lines.append("⛔ KEINE Aenderungen an anderen Dateien")
...
def main():
    ...
    if args.registry:
        ...
        subprocess.run(['python3', 'mas-engineer/tools/dev_registry_merge.py', '--findings', json.dumps(f_item), '--registry', args.registry, '--project', args.project], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    
    if args.json:
        print(json.dumps(stats, indent=2))
    else:
        print("Agent erstellt: " + stats['name'])
        print("  prompt: " + str(stats['prompt_len']) + " Z")
        print("  instructions: " + str(stats['instructions_len']) + " Z")
        print("  BP-Regeln: " + str(stats['bp_rules']))
        print("  Datei: " + stats['output'])
