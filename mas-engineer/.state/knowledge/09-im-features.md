  ...
  Erkenne UND dokumentiere JEDEN gefundenen Type:

  ### Type A — Settings-Optimierung
  ...
  - C2: Unklarer Ablauf → Agent asks after Reihenfolge → nummerieren
  - C3: Missing boundaries → Agent overschreitet Scope → boundaries add
  - C4: Veraltete Referenz → Referenzen auf not-existente files → updaten

  ### Type D — Workflow-Optimierung (dev-mas-engineer.yaml)
  ...
  - D4: Unklare question → User answert "?" → make precise

  ### Type E — Detectionsmuster
  Erkenne: Muster im Intention-Parser.
  - E1: Missing Detection → User nutzt Command without Match → Muster hinzu
  - E2: Falsche Detection → Muster matcht wrong → anpassen
  ...
  - F2: Sortierung → Most frequent not oben → after Nutzung sort
  ...
  - J2: Config-Value obsolete → Referenzen stimmen not → updaten
  ...
  - O2: Referenz auf not-existente file → grep finds import/file/path not im Workspace
  ...
  ### Type Q — Agent-Kalibrierung (via Session-DB + Settings-Comparison)
  ...
  - S1: Lowster Score → ∅ allr Metriken am schlechtesten → optimize
  ...
  - T1: Hohe Negative-Quote → >20% User-answers are negativ → Response-Quality untersuchen
  - T2: Many follow-ups → >10% User asks after ("was?", "wie?", "again") → answers make precise
  - T3: Positivee Stimmung → meist "ja", "weiter", "danke" → ✅ Quality gut

  ### Type U — Erfolgsrate-Trend (via Session-DB about Wochen)
  Erkenne: Trend.
  ...
  ### Type AA — duration-Beforesage (via Session-DB + Verlauf)
  ...
  ### Type FW — Framework-Agent-Optimierung (via dev_agent_doctor.py)
  ...
  - FW3: Struktur inconsistent → Input/Output, Constitution-Referenz add
  ...
  - CC1: OOM/MemoryError erkannt → Sub-Agent cancelled → timeout reduce ODER Task splitten
  ...
  - DD4: Tool-Syntax-Error in Session → Tool-Aufruf korrigieren

  ### Type EE — Session-Korruption (via sqlite3 + changes.json)
  Erkenne: DB-Probleme.
  ...
  Erkenne: Backup junk.
  - HH1: >50 Backup-Verzeichnisse in .backups/ → User ask: Backups clean?
  ...
  ### Type JJ — Installations-Drift (via diff workspace vs ~/.config/goose/recipes/)
  Erkenne: Installations-Deviation.
  - JJ1: dev-mas-engineer.yaml unterschiedlich → update.sh --mas execute
  - JJ2: Sub-Agent missing in ~/.config/goose/recipes/sub/ → kopieren
  - JJ3: Tool missing in ~/.config/goose/recipes/mas-engineer-tools/ → dev_build.sh + install
  - JJ4: Doc missing in ~/.config/goose/docs/mas-engineer/ → kopieren

  ### Type MM — YAML-Struktur-Validierung (via python3 yaml.safe_load)
  ...
  - MM1b: constitution: zeigt auf falsche file
  ...
  ### Type LL — User-Interaktions-Muster (via session-reader include_messages=true)
  ...
  - LL3: User-Zufriedenheits-Muster
    ...
    beforeschlag: "No Change needed — can als Positive-Example dienen"

  - LL4: Session-Cancel-Muster
  ...
  Erkenne: Missing SOT-Hardening-Referenzen in Agent-YAMLs.
  FOR jedes sub_mas-*.yaml in recipe/sub/:
    CHECK: grep -cE "SOT|configs.mas-self" $f = 0?
      → KK1: SOT-Referenz missing — "Nicht an SOT angebunden"
    ...
    SCORE: Number der gefundenen Ref-Kategorien (0-5)
